    ...
    def __init__(self,
        ...
        if missing_trans:
            if asset is not None:
                ...
                for col in TRANSFORMER_FEAT_COLS:
                    df[col] = 0.0
        # Optional ablation: disable Transformer forecast inputs for PPO.
        # This keeps the same observation dimension but removes Transformer signal.
        self.no_transformer_signal = os.getenv("NO_TF_SIGNAL", "0") == "1"

        if self.no_transformer_signal:
            for col in TRANSFORMER_FEAT_COLS:
                if col in df.columns:
                    df[col] = 0.0

            logger.warning(
                f"[{asset or 'ENV'}] NO_TF_SIGNAL=1 → Transformer inputs zeroed for PPO ablation."
            )
...
# RL step-level close/price series so dashboard candles are always available.
    ...
    def step(self, action):
        ...
        def _emit_completed_trade(er: str):
            ...
            # Use portfolio value at entry as capital base, not fixed initial capital.
            notional = portfolio_before * pf * lev

            # Direction-aware price return.
            # LONG: profit when exit > entry.
            # SHORT: profit when exit < entry.
            side_sign = 1.0 if side == 1 else -1.0
            gross_return = side_sign * (xp - ep) / ep

            # Direction-correct trade PnL.
            gross_pnl = gross_return * notional

            # Transaction costs are always subtracted.
            tc_total = float(self._tacc_tc)
            net_pnl = gross_pnl - tc_total

            portfolio_after = portfolio_before + net_pnl

            # Return on leveraged notional.
            ret_pct = net_pnl / notional if notional > 0 else 0.0

            # Keep this column for debugging; now it should match gross_pnl.
            price_move_pnl_estimate = gross_pnl
            # Validation check: this should never happen after the fix.
            if side == 1 and xp < ep and gross_pnl > 0:
                logger.warning(
                    f"[{self._asset}] PnL sign error: LONG exit < entry but gross_pnl > 0 "
                    f"trade_id={self._trade_id} entry={ep} exit={xp} gross_pnl={gross_pnl}"
                )

            if side == -1 and xp > ep and gross_pnl > 0:
                logger.warning(
                    f"[{self._asset}] PnL sign error: SHORT exit > entry but gross_pnl > 0 "
                    f"trade_id={self._trade_id} entry={ep} exit={xp} gross_pnl={gross_pnl}"
                )
